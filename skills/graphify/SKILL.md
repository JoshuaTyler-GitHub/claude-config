...
mkdir -p .graphify
"$PYTHON" -c "import sys; open('.graphify/.graphify_python', 'w', encoding='utf-8').write(sys.executable)"
...
**In every subsequent bash block, replace `python3` with `$(cat .graphify/.graphify_python)` to use the correct interpreter.**
...
if code_files:
    ...
    Path('.graphify/.graphify_ast.json').write_text(json.dumps(result, indent=2, ensure_ascii=False), encoding=\"utf-8\")
    print(f'AST: {len(result[\"nodes\"])} nodes, {len(result[\"edges\"])} edges')
else:
    Path('.graphify/.graphify_ast.json').write_text(json.dumps({'nodes':[],'edges':[],'input_tokens':0,'output_tokens':0}, ensure_ascii=False), encoding=\"utf-8\")
...
if cached_nodes or cached_edges or cached_hyperedges:
    Path('.graphify/.graphify_cached.json').write_text(json.dumps({'nodes': cached_nodes, 'edges': cached_edges, 'hyperedges': cached_hyperedges}, ensure_ascii=False), encoding=\"utf-8\")
Path('.graphify/.graphify_uncached.txt').write_text('\n'.join(uncached), encoding=\"utf-8\")
...
chunks = sorted(glob.glob('.graphify/.graphify_chunk_*.json'))
...
Path('.graphify/.graphify_semantic_new.json').write_text(json.dumps({
...
new = json.loads(Path('.graphify/.graphify_semantic_new.json').read_text(encoding=\"utf-8\")) if Path('.graphify/.graphify_semantic_new.json').exists() else {'nodes':[],'edges':[],'hyperedges':[]}
...
cached = json.loads(Path('.graphify/.graphify_cached.json').read_text(encoding=\"utf-8\")) if Path('.graphify/.graphify_cached.json').exists() else {'nodes':[],'edges':[],'hyperedges':[]}
new = json.loads(Path('.graphify/.graphify_semantic_new.json').read_text(encoding=\"utf-8\")) if Path('.graphify/.graphify_semantic_new.json').exists() else {'nodes':[],'edges':[],'hyperedges':[]}
...
Path('.graphify/.graphify_semantic.json').write_text(json.dumps(merged, indent=2, ensure_ascii=False), encoding=\"utf-8\")
...
ast = json.loads(Path('.graphify/.graphify_ast.json').read_text(encoding=\"utf-8\"))
sem = json.loads(Path('.graphify/.graphify_semantic.json').read_text(encoding=\"utf-8\"))
...
Path('.graphify/.graphify_extract.json').write_text(json.dumps(merged, indent=2, ensure_ascii=False), encoding=\"utf-8\")
...
extraction = json.loads(Path('.graphify/.graphify_extract.json').read_text(encoding=\"utf-8\"))
detection  = json.loads(Path('.graphify/.graphify_detect.json').read_text(encoding=\"utf-8\"))
...
Path('.graphify/GRAPH_REPORT.md').write_text(report, encoding=\"utf-8\")
...
Path('.graphify/.graphify_analysis.json').write_text(json.dumps(analysis, indent=2, ensure_ascii=False), encoding=\"utf-8\")
...
extraction = json.loads(Path('.graphify/.graphify_extract.json').read_text(encoding=\"utf-8\"))
detection  = json.loads(Path('.graphify/.graphify_detect.json').read_text(encoding=\"utf-8\"))
analysis   = json.loads(Path('.graphify/.graphify_analysis.json').read_text(encoding=\"utf-8\"))
...
Path('.graphify/GRAPH_REPORT.md').write_text(report, encoding=\"utf-8\")
Path('.graphify/.graphify_labels.json').write_text(json.dumps({str(k): v for k, v in labels.items()}, ensure_ascii=False), encoding=\"utf-8\")
...
extract = json.loads(Path('.graphify/.graphify_extract.json').read_text(encoding=\"utf-8\"))
...
Before running any subcommand below (`--update`, `--cluster-only`, `query`, `path`, `explain`, `add`), check that `.graphify_python` exists. If it's missing (e.g. user deleted `.graphify/`), re-resolve the interpreter first:

```bash
if [ ! -f .graphify/.graphify_python ]; then
    GRAPHIFY_BIN=$(which graphify 2>/dev/null)
...
When `.graphify/graph.json` already exists and the user asks a question about the corpus, run the query directly:
